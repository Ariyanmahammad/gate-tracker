...
export function testStats(state: AppState) {
  ...
  const pcts = tests.filter((t) => t.totalMarks && t.totalMarks > 0)
  ...
  const accs = tests.filter((t) => (t.correct || 0) + (t.wrong || 0) > 0)
...
export function longestStreak(state: AppState, tasks: PlannedTask[]): number {
  ...
  for (const d of dates) {
    const pct = dayCompletionPct(state, byDate.get(d)!);
    if (pct >= 50) { cur++; longest = Math.max(longest, cur); } else { cur = 0; }
  }
  return longest;
}
...
export function subjectStats(state: AppState, tasks: PlannedTask[]) {
  ...
  return [...bySubject.entries()].map(([subject, subjTasks]) => {
    ...
    return {
      subject, totalTasks: subjTasks.length, completed, partial, skipped,
      completionPct: pct, pyqsSolved: pyqs,
